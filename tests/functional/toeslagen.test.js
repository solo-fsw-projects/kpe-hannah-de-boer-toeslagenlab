...
describe('Toeslagen Module Tests', () => {
    ...
    describe('End-to-End Flow', () => {
        test('applies custom expense correctly', async () => {
            // Initial load
            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'oktober 2024', '', 100);
            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'oktober 2024', '', 100);
    ...
    describe('Navigation and State', () => {
        test('maintains state when navigating between months', async () => {
            ...
            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'november 2024', '', 100);
            window.toeslagen.applyIncomes();
            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'november 2024', '', 100);
            await new Promise(resolve => setTimeout(resolve, 1000));
    ...
    describe('Progress Bar', () => {
        ...
        test('positions circle correctly for different months', async () => {
            ...
            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'oktober 2024', '', 100); // 1 of 12
            expect(getPosition()).toBe('calc(0\% - 18px)');

            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'april 2025', '', 100); // 7 of 12
            expect(getPosition()).toMatch(/calc\(35\.\d+\% - 18px\)/);

            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'maart 2026', '', 100); // 12 of 12
    ...
    describe('Toeslag Calculation', () => {
        test('applies toeslag percentage with proper rounding', async () => {
            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'oktober 2024', 'Huurtoeslag', 80);
            await window.toeslagen.runOnNewSlide(sheetUrl, true, 1000, 'oktober 2024', 'Huurtoeslag', 80);
